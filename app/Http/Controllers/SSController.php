...
class SSController extends Controller
{
    ...
    {
        return view('elecciones.SS',  ['casilla' => $casilla]);
    }

    public function resultados() {
        $this->validar();
        $mujeres=DB::select('CALL votos_MujeresSS()');
        $hombres=DB::select('CALL votos_HombresSS()');
        $blanco=DB::select("SELECT COUNT(*) AS blanco FROM VOTOSSS WHERE STATUS='B'");      
        $ilegibles=DB::select("SELECT COUNT(*) AS ilegible FROM VOTOSSS WHERE STATUS='I'");
        $nulos=DB::select("SELECT COUNT(*) AS nulos FROM VOTOSSS WHERE STATUS='N'");
        $bol=DB::select("SELECT COUNT(*) as boletas FROM ss");
        $bolRNU=DB::select("SELECT COUNT(*) as boletas FROM rnu WHERE eleccion='SS'");
        ...
        return view('resultados.SS', $datos);
    }

    public function validar()
    {
        $votos=SS::where('status', 'N')->get()->values();
        //dd($votos);
        if($votos){     
            ...
            for($i=0; $i<count($votos); $i++){                                
                ...
                for($j=1; $j<=2; $j++){
                    ...
                    SS::where('id', $votos[$i]['id'])->update(['status'=>'S']);
